...
export default function Home({ products, categories }) {
  ...
  return (
    <div className="min-h-screen bg-[#030305] text-white overflow-x-hidden font-sans selection:bg-cyan-500/50 relative">
      ...
      {/* 🔥🔥🔥 BACKGROUND SYSTEM (พื้นหลังตาราง Cyberpunk) 🔥🔥🔥 */}
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        {/* พื้นหลังดำ */}
        <div className="absolute inset-0 bg-[#020202]"></div>
        
        {/* ตาราง Grid ที่ขยับได้ */}
      ...
      {categories.length > 0 && (
          <div className="max-w-8xl mx-auto px-6 mb-12 relative z-10">
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                <button onClick={() => setSelectedCategory('All')} className={`flex items-center gap-2 px-6 py-3 rounded-xl border transition-all whitespace-nowrap font-bold ${selectedCategory === 'All' ? 'bg-cyan-600 border-cyan-500 text-white shadow-[0_0_15px_rgba(6,182,212,0.4)]' : 'bg-[#0a0a0a] border-white/10 text-gray-400 hover:border-cyan-500/50'}`}>
                    <LayoutGrid size={18}/> ทั้งหมด
                </button>
                {categories.map((cat) => (
                    <button key={cat.id} onClick={() => setSelectedCategory(cat.name)} className={`flex items-center gap-3 px-5 py-2 rounded-xl border transition-all whitespace-nowrap group ${selectedCategory === cat.name ? 'bg-white/10 border-cyan-500 text-white' : 'bg-[#0a0a0a] border-white/10 text-gray-400 hover:border-white/30'}`}>
                    ...
                    <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} scale={1.02} transitionSpeed={2000} className="h-full">
                        <div className="group h-full relative bg-[#0a0a0a] border border-white/5 rounded-3xl overflow-hidden hover:border-cyan-500/50 transition-all duration-500 shadow-2xl hover:shadow-[0_0_40px_rgba(6,182,212,0.15)]">
                            ...
                            <div className="h-60 overflow-hidden relative">
                              ...
                              {product.discount > 0 && (<div className="absolute top-3 right-3 z-30"><span className="bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg">-{product.discount}%</span></div>)}
                              <div className="absolute top-3 left-3 z-20"><span className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1 px-3 py-1 rounded backdrop-blur-md border ${product.type === 'TOPUP' ? 'bg-green-500/10 border-green-500/50 text-green-400' : 'bg-purple-500/10 border-purple-500/50 text-purple-300'}`}><Cpu size={10}/> {product.type === 'TOPUP' ? 'AUTO' : 'ID'}</span></div>
                            </div>
                            ...
                            <div className="p-6 relative z-20 flex flex-col h-auto">
                              ...
                              <div className="mt-auto flex justify-between items-end border-t border-dashed border-white/10 pt-4">
                                ...
                                <Link href={`/product/${product.id}`}><button className="bg-white text-black px-5 py-2.5 rounded-xl text-xs font-black transition-all hover:bg-cyan-400 hover:scale-105 active:scale-95 flex items-center gap-1 group/btn shadow-[0_0_15px_rgba(255,255,255,0.1)]">BUY <ChevronRight size={14} className="group-hover/btn:translate-x-0.5 transition-transform"/></button></Link>
                              </div>
                            </div>
                        </div>
                    </Tilt>
      ...
      <footer className="border-t border-cyan-900/30 bg-[#020202] py-16 text-center relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-200 to-gray-500 mb-4">NEONSTORE SYSTEMS</h2><p className="text-gray-500 text-xs">© 2025 All rights reserved.</p>
      </footer>
...
export async function getServerSideProps() {
  try {
    ...
    const products = await prisma.product.findMany({ where: { isRecommended: true }, orderBy: { createdAt: 'desc' } });

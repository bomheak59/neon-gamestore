...
import { Zap, Gamepad2, ShieldCheck, ChevronRight, Monitor, CreditCard, MessageSquareQuote, Bell, Cpu, Sparkles, CheckCircle, Clock, Headphones } from 'lucide-react'; // ลบ ShoppingCart, Search ออกจาก import
    ...
    <div className="min-h-screen bg-[#030305] text-white overflow-x-hidden font-sans selection:bg-cyan-500/50 relative">
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#050505] to-[#020202]">
        <motion.div 
            ...
            className="absolute top-[-20%] left-[20%] w-[800px] h-[800px] bg-cyan-600/10 rounded-full blur-[150px]" 
        />
        <motion.div 
            animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.5, 0.3] }} 
            transition={{ duration: 10, repeat: Infinity, delay: 1 }}
            className="absolute top-[-10%] right-[10%] w-[600px] h-[600px] bg-purple-600/10 rounded-full blur-[150px]" 
        />
      </div>
      ...
      <nav className="sticky top-0 z-50 border-b border-cyan-500/10 bg-[#050505]/80 backdrop-blur-2xl transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 h-24 flex items-center justify-between">
          ...
          <div className="hidden xl:flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-gray-400">
            ...
            {/* ลิงก์เติมเกม */}
            <Link href="/topup">
              <button className="flex items-center gap-2 px-4 py-3 hover:text-green-400 hover:bg-white/5 rounded-lg transition-all cursor-pointer">
                  <CreditCard size={18}/> บริการเติมเกม
              </button>
            </Link>
          ...
          {/* 🔥 ส่วนค้นหาและรถเข็น ถูกลบออกแล้ว 🔥 */}
          <div className="flex items-center gap-5">
              {/* (พื้นที่ว่างไว้ เผื่ออยากใส่อะไรเพิ่มในอนาคต) */}
          </div>
        </div>
      </nav>
      ...
      <div className="relative pt-32 pb-48 text-center px-4 z-10 overflow-hidden perspective-container">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <div className="mb-6">
             <div className="inline-block bg-gradient-to-r from-red-600 to-pink-600 text-white px-6 py-2 rounded-full text-sm font-bold shadow-lg animate-bounce-slow transform -rotate-2 border border-white/10">
                ⚡ DISCOUNT UP TO 70% TODAY
             </div>
          </div>
          ...
          <h1 className="text-5xl sm:text-7xl md:text-8xl font-black tracking-tighter mb-6 leading-none drop-shadow-2xl">
            ...
            <div className="min-h-[1.2em] text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600">
                <Typewriter
                  options={{
                    strings: ['NEONSTORE', 'GAME ID SHOP', 'TOPUP SERVICE'],
                    autoStart: true,
                    loop: true,
                    delay: 75,
                    deleteSpeed: 50,
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-3xl mx-auto">
             <motion.div whileHover={{ scale: 1.05 }} className="flex flex-col items-center justify-center bg-white/5 border border-white/10 hover:bg-white/10 hover:border-cyan-500/50 p-6 rounded-2xl backdrop-blur-sm transition-all group">
                <div className="bg-cyan-500/20 p-4 rounded-full mb-3"><CheckCircle size={28} className="text-cyan-400"/></div>
                <span className="text-base font-bold text-white">ผู้ขายยืนยันตัวตน</span><span className="text-xs text-gray-500 mt-1">Verified Seller</span>
             </motion.div>
             <motion.div whileHover={{ scale: 1.05 }} className="flex flex-col items-center justify-center bg-white/5 border border-white/10 hover:bg-white/10 hover:border-yellow-500/50 p-6 rounded-2xl backdrop-blur-sm transition-all group">
                <div className="bg-yellow-500/20 p-4 rounded-full mb-3"><Clock size={28} className="text-yellow-400"/></div>
                <span className="text-base font-bold text-white">ส่งสินค้าอัตโนมัติ</span><span className="text-xs text-gray-500 mt-1">Auto Delivery 24/7</span>
             </motion.div>
             <motion.div whileHover={{ scale: 1.05 }} className="flex flex-col items-center justify-center bg-white/5 border border-white/10 hover:bg-white/10 hover:border-green-500/50 p-6 rounded-2xl backdrop-blur-sm transition-all group">
                <div className="bg-green-500/20 p-4 rounded-full mb-3"><Headphones size={28} className="text-green-400"/></div>
                <span className="text-base font-bold text-white">บริการหลังการขายตลอด 24 ชม.</span><span className="text-xs text-gray-500 mt-1">Support Team</span>
             </motion.div>
          </div>
        </div>
      </div>
